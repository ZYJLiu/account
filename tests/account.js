...
describe("Begin Test", () => {
  ...
  async function createList(owner, name, capacity = 16) {
    const [listAccount, bump] = await anchor.web3.PublicKey.findProgramAddress(
      ["list", owner.key.publicKey.toBytes(), name.slice(0, 32)],
      mainProgram.programId
    );
  ...
  it("Revenue - Receive Payment", async () => {
    ...
    const balance7 =
      (await getAccountBalance(payer.key.publicKey)) / LAMPORTS_PER_SOL;
  ...
  it("Expense - Send Payment", async () => {
    ...
    const balance9 =
      (await getAccountBalance(receiver.key.publicKey)) / LAMPORTS_PER_SOL;
